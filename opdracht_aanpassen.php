...
<?php
    ...
    $subject = User::getSubjectById($subject_id)['name'];
    $className = User::getClassById($class_id);

    if(isset($_GET['delete_id'])) {
        $deleteOpdracht = new User();
        $deleteOpdracht->setAssignment_id($_GET['delete_id']);
        $deleteOpdracht->deleteAssignment();

        header('Location: /bouwsteenappleerkrachten/opdracht_nederlands.php?class_id=' . $class_id . '&subject_id=' . $subject_id);
    }

    if(!empty($_POST)) {
        if(empty($_POST['titel']) || empty($_POST['beschrijving']) || empty($_POST['teverdienenmunten']) || empty($_POST['einddatum'])) {
            $error = "Opdracht is niet aangepast.";
        } else { 
        $title = $_POST['titel'];
        ...
        $opdrachtAanpassen = new User();
        $opdrachtAanpassen->setTitle($title);
        $opdrachtAanpassen->setDescription($description);
        $opdrachtAanpassen->setReward($reward);
        $opdrachtAanpassen->setDate($date); 
        $opdrachtAanpassen->setSubject_id($subject_id);
        $opdrachtAanpassen->setClass_id($class_id);
        $opdrachtAanpassen->setAssignment_id($assignment_id);
...
<body>
    ...
    <section>
        ...
        <form action="#" method="POST" id="opdrachtform" enctype="multipart/form-data" onsubmit="opdrachtAanpassen(event)">
            <div>
                <p>Klas:</p>
            ...
            <div>
                ...
                <input type="date" value="<?php echo $assignment['due_date']; ?>" name="einddatum" placeholder="2022-03-06" min="2022-03-06" max="2022-06-30">
            </div>

            <div>
                <p><label for="teverdienenmunten">Te verdienen munten:</label></p>
                <input type="number" value="<?php echo $assignment['reward']; ?>" name="teverdienenmunten" min="0" placeholder="10">
            </div>

            <div>
                <p><label for="titel">Titel:</label></p>
                <input type="text" value="<?php echo htmlspecialchars($assignment['name']); ?>" name="titel" placeholder="Titel van de opdracht">
            </div>

            <div>
                <p><label for="beschrijving">Beschrijving:</label></p>
                <input type="text" value="<?php echo htmlspecialchars($assignment['description']); ?>" class="beschrijving" name="beschrijving" placeholder="Beschrijving van de opdracht">
            </div>
            ...
            <a style="background-color: #FF0000; color: white;" href="opdracht_aanpassen.php?delete_id=<?php echo $assignment_id; ?>&class_id=<?php echo $class_id; ?>&subject_id=<?php echo $subject_id; ?>" class="btn">Verwijder</a>
